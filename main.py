    ...
    <html lang="en">
        <head>
            ...
            <style>
                ...
                .chute-review-name-search-results {{
                    margin-top: var(--cf-space-2);
                }}
                .chute-review-corrected-note {{
                    font-style: italic;
                }}
...
# Both legacy actions ("competitive_bidirectional_*") are the retired
# Flow A this epic already consolidated away (dd58bc6). Their routes have
# since been deleted outright (2026-08-30 delink-then-delete follow-up) --
# _pricing_job_detail_url never returns a URL for either one now, so both
# render as a plain, non-broken label here, same as any other unlinked
# row; the 6 historical preview rows lost their drill-in page along with
# the apply rows, which never had one.
...
# POST /pricing/job-preview and the entire /pricing/competitive-job/*
# route family (Flow A) were deleted 2026-08-30 -- delinked first (see
# _pricing_job_detail_url above), confirmed 6 historical PricingJob rows
# still using competitive_bidirectional_preview render as a plain label
# with no broken link, then removed here. The helper functions that
# existed solely to support this flow (parse_bidirectional_price_report,
# build_literal_low_verified_preview, and the small variant/condition
# matching cluster above _PRICING_ACTION_LABELS) were deliberately left
# in place rather than deleted along with the routes -- they're now dead
# code, flagged separately, not silently removed beyond what was asked.
