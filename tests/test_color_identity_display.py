...
def setup_db(tmp_path, monkeypatch):
    db = create_engine(f"sqlite:///{tmp_path / 'color_identity.db'}")
...
def add_card(session, *, name="Forest", color_identity=None, status="available"):
    batch = Batch(batch_code=f"B-{name}-{color_identity}")
    session.add(batch)
    session.flush()
    card = InventoryCard(
        batch_id=batch.id, name=name, set_code="SET", collector_number="1",
        scryfall_id="sf-1", condition="LP", condition_id="LP", finish="normal",
        finish_id="NF", language_id="EN", status=status, color_identity=color_identity,
    )
    session.add(card)
    session.commit()
    return card
...
def test_badge_renders_a_chip_per_letter_in_wubrg_order():
    assert main._color_identity_badge("WU") == (
...
def test_edit_page_shows_color_badge_in_header(tmp_path, monkeypatch):
    db = setup_db(tmp_path, monkeypatch)
    with Session(db) as session:
        card = add_card(session, color_identity="UB")
        card_id = card.id
...
def test_unresolved_color_identity_shows_no_badge(tmp_path, monkeypatch):
    db = setup_db(tmp_path, monkeypatch)
    with Session(db) as session:
        add_card(session, color_identity=None)
    response = TestClient(main.app).get("/inventory?show_all=true")
    assert response.status_code == 200
